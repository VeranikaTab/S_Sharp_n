...
void SrArifmColumn(int[,] arr)
{
    ...
    for (int j = 0; j < column; j++)
    {
        ...
        Console.WriteLine($"Cреднее арифметическое элементов в {j + 1} столбце: {Math.Round((sum / row), 1),4} ");
    }
}
...
void SrArifmColumn(int[,] arr)
{
    ...
    Console.Write($"{Math.Round((sum / row), 1),4} | {Math.Round((sum1 / row), 1),4} | ");
    Console.Write($"{Math.Round((sum2 / row), 1),4} | {Math.Round((sum3 / row), 1),4} ");
}
